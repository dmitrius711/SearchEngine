...
projectData ConverterJSON::GetNameAndVersion()
{
    ...
    std::ifstream configFile("C:\\SearchEngine2\\json_files\\config.json");
...
{
    std::ifstream requestFile("C:\\SearchEngine2\\json_files\\requests.json");
...
void ConverterJSON::putAnswers(const std::vector<std::vector<RelativeIndex>> &answers)
{
    nlohmann::json ans;
    int ind = 1;
    std::ofstream answersFile("C:\\SearchEngine2\\json_files\\answers.json", std::ios::trunc);
